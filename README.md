...
Download (opendata.swiss)
  → Geocode (Mapbox API, scripts/geocode_mapbox.py)
...
2. **Geocode** — `scripts/geocode_mapbox.py` queries the Mapbox Geocoding API at 50 req/s, building each query as `"{street}, {postal_code} {locality}, Switzerland"`; a local cache (`geocode_cache.json`) makes runs resumable; fallback scripts run at 20 and 10 req/s when needed
